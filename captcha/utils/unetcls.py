...
def get_unetcls():
    ...
    conv_0_down = conv_block(inputs, num_kernels[0], kernel_size, strides, padding, activation, dropout, data_format,
                             bn)
    pool_0 = MaxPooling2D(pool_size=pool_size,
                          data_format=data_format)(conv_0_down)
    ...
    pool_1 = MaxPooling2D(pool_size=pool_size,
                          data_format=data_format)(conv_1_down)
    ...
    pool_2 = MaxPooling2D(pool_size=pool_size,
                          data_format=data_format)(conv_2_down)

    # level 3
    conv_3_down = conv_block(pool_2, num_kernels[3], kernel_size, strides, padding, activation, dropout, data_format,
                             bn)
    pool_3 = MaxPooling2D(pool_size=pool_size,
                          data_format=data_format)(conv_3_down)

    # level 4
    conv_4 = conv_block(pool_3, num_kernels[4], kernel_size,
                        strides, padding, activation, dropout, data_format, bn)
    ...
    # level 3
    concat_3 = up_concat_block(
        conv_4, conv_3_down, pool_size, concat_axis, data_format)
    ...
    conv_0_up = conv_block(concat_0, num_kernels[0], kernel_size, strides, padding, activation, dropout, data_format,
                           bn)
    final_conv = Convolution2D(1, 1, strides=strides, activation=activation, padding=padding,
                               data_format=data_format)(conv_0_up)
    final_conv = Flatten()(final_conv)
    final_conv = Dense(128, activation=activation,
                       kernel_initializer='he_uniform')(final_conv)
    final_conv = Dense(1, activation=final_activation)(final_conv)
