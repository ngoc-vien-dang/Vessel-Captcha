...
def main(args):
    wnetseg_prediction_dir = args.wnetseg_prediction_dir
    patch_size = args.patch_size
    model_filepath = args.model_filepath
    train_metadata_filepath = args.train_metadata_filepath
    wnetseg_pnetcl_filepath = args.wnetseg_pnetcl_filepath
    ...
    for i, j in enumerate(input_list):
        ...
        print('Loading wnetseg prediction...')
        prediction_mat = load_nifti_mat_from_file(prediction_list[i])
        # pnetcls prediction matrix
        ...
        results = prob_mat*prediction_mat
        # Save weak annotation
        create_and_save_nifti(results, wnetseg_pnetcl_filepath +
                              j.split(os.sep)[-1].split('_')[0]+'_wnetseg_pnetcls.nii')
        print('done')
    print('DONE')

def parse_arguments(argv):
    parser = argparse.ArgumentParser()
    parser.add_argument("--wnetseg_prediction_dir", type=str,
    ...
    parser.add_argument("--wnetseg_pnetcl_filepath", type=str,
                        help='The filename path for saving vessel segmentation applied by the filter.')
